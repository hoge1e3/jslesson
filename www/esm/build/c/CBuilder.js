...
import wget from "./../../fs2/wget.js";
import compile from "./../../ctrans/compiler.js";
...
import FileBrowser from "./../../fsui/FileBrowser.js";
...
var CBuilder=function (prj, dst, ide) {
    this.prj=prj;// TPRC
...
p.genHTML=function (f,options) {
    ...
    `));
    $(head).append($("<script>").text("window.runtimePath=BitArrow.runtimePath;"));
...
p.addMenu=function (Menu) {
    ...
    const runAtBrowser=async ()=>{
        await this.ide.run({runAt:"browser"});
    };
    let runAts=[runAtBrowser, runAtDocker];
    let menus=[
    ...
    if (WebSite.runAtServerDefault){
        runAts=[runAtDocker, runAtBrowser];
    ...
    Menu.appendMain(
        {label:"実行",id:"runC",after:$("#fileMenu"),sub:menus
        }
    );
