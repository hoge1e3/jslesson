  ...
  initCalibration: function () {
    ...
    try {
      ...
      self.calibratedKeys.forEach(function (k, i) {
        var c = self.calibration[k];
        if (typeof c.rawkey === "string" && typeof c.sgn === "number") {
        } else {
          throw "Invalid data";
        }
      });
...
root.sensorNotWorking = function () {
  ...
  var text2 = $("<h3>").html(
    "以下のボタンからセンサ情報へのアクセスを許可していただくと、改善する可能性があります",
  );
  ...
  window.$(function () {
    window.addEventListener(
      ...
      function (evt) {
        ...
        self.working =
          self.working || self.x !== 0 || self.y !== 0 || self.z !== 0;
      },
      true,
    );
...
root.GPS.init = function () {
  if (this.initialized) return this;
  window.$(function () {
    this.gps = window.navigator.geolocation.getCurrentPosition(
      function (position) {
  ...
  window.$(function () {
    window.document.addEventListener(
      ...
      function (evt) {
        ...
        var height =
          window.$("#canvas").context.documentElement.clientHeight / 2;
    ...
    window.document.addEventListener(
      ...
      function (evt) {
        var x = 0,
          y = 0;
        var width =
          window.$("#canvas").context.documentElement.clientWidth / 2 || 0;
        var height =
          window.$("#canvas").context.documentElement.clientHeight / 2 || 0;
