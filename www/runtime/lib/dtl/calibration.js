  ...
  setInterval(function () {
    ...
    switch (state) {
      ...
      case 7:
        ...
        if (cnt > 3000) {
          if (
            a.verifyCalibration("y", -1) &&
            g.verifyCalibration("pitch", -1)
          ) {
            state++;
          } else state = 4;
          cnt = 0;
      ...
      case 12:
        ...
        if (gc) {
          ...
          cv.fillRect(
            W / 2 + (g.normalize(gc.roll - rollBase) / 180) * (W / 2),
            H / 2 + (g.normalize(gc.pitch - pitchBase) / 180) * (H / 2),
            10,
            10,
          );
          cv.beginPath();
          cv.moveTo(
            W / 2 + (Math.cos(-rad(gc.yaw)) * W) / 2,
            H / 2 + (Math.sin(-rad(gc.yaw)) * H) / 2,
          );
          cv.lineTo(
            W / 2 - (Math.cos(-rad(gc.yaw)) * W) / 2,
            H / 2 - (Math.sin(-rad(gc.yaw)) * H) / 2,
