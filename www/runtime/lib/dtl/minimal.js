MinimalParser = (function () {
  ...
  var reg_num =
    /^(([0０][bBｂＢ][01０１]+)|([0０][xｘXＸ][0-9０-９a-fA-Fａ-ｆＡ-Ｆ]+)|([0-9０-９]+([.．]([0-9０-９])+)?))([a-zA-Z_$\?？ーぁ-んァ-ヶ々〇〻\u3400-\u9FFF\uF900-\uFAFF\uD840-\uD87F\uDC00-\uDFFF][a-zA-Z_$\?？ーぁ-んァ-ヶ々〇〻\u3400-\u9FFF\uF900-\uFAFF\uD840-\uD87F\uDC00-\uDFFF]*)?/; //数字を表す正規表現
  //var reg_num=/^[0-9０-９]+/;
  ...
  statement_list = statement
    ...
    .ret(function (_stmts) {
      ...
      if (useGenerator()) {
        return extend(
          [
            "return AsyncByGenerator.toVal((function*() {",
            stmts,
            "return ",
            last,
            "}).apply(this));",
  ...
  parser.parseAsNode = function (str, options) {
    ...
    ctx = context();
    ctx.usegen =
      AsyncByGenerator.supportsGenerator && input.indexOf("NOGENERATOR") < 0;
