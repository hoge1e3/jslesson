    ...
    extend(Parser.prototype, {
      ...
      and: function (next) {
        ...
        if ($.options.verboseFirst) {
          console.log(
            "Created aunify name=" + res.name + " tbl=" + $.dispTbl(ntbl),
          );
        }
        return res;
      },
      ...
      ret: function (next) {
        ...
        if ($.options.verboseFirst) {
          console.log(
            "Created runify name=" + res.name + " tbl=" + $.dispTbl(ntbl),
          );
        }
        return res;
      },
	...
	*/
      first: function (space, ct) {
        ...
        if (typeof ct == "string") {
          ...
          return Parser.fromFirst(space, tbl).setName(
            "(fst " + this.name + ")",
          );
          //        		this._first={space: space, chars:ct};
        } else if (ct == null) {
          return Parser.fromFirst(space, { ALL: this }).setName(
            "(fst " + this.name + ")",
          );
      ...
      unifyFirst: function (other) {
        ...
        function mergeTbl() {
          ...
          for (var k in t2) {
            /*if (d) console.log("t2.k="+k);*/ keys[k] = 1;
          }
          delete keys.ALL;
          if (tbl.ALL || t2.ALL) {
        ...
        mergeTbl();
        var res = Parser.fromFirst(this._first.space, tbl).setName(
          "(" + this.name + ")U(" + other.name + ")",
        );
        if ($.options.verboseFirst)
          console.log(
      ...
      or: function (other) {
        ...
        if (
          ...
          other._first &&
          this._first.space &&
          this._first.space === other._first.space
        ) {
    ...
    Parser.fromFirst = function (space, tbl) {
      ...
      var res = Parser.create(function (s0) {
        ...
        if ($.options.traceFirstTbl) {
          console.log(
            this.name + ": first=" + f + " tbl=" + (tbl[f] ? tbl[f].name : "-"),
          );
        }
        if (tbl[f]) {
