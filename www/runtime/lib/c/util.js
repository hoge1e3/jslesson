...
(function () {
    ...
    _global.LocalVariables.prototype = {
        __getAddress: function __getAddress(name, vtype) {
    ...
    _global.StructObj = function StructObj(vtype) {
        ...
        if (vtype instanceof BA_C.CType.Struct) {
            //TODO: CTYPE_NAME
            var t = this;
    ...
    _global.copyStruct = function copyStruct(src) {
        ...
        for (var k in src) {
            ...
            if (isStruct(src[k])) res[k] = copyStruct(src[k]);else res[k] = src[k];
        }
        return res;
        function isStruct(o) {
    ...
    _global.copyArray = function copyArray(dst, src, vtype) {
        ...
        for (var i = 0; i < vtype.length; i++) {
            ...
            if (e instanceof BA_C.CType.Struct) _global.copyStruct2(dst.offset(i).read(), v, e);else if (e instanceof BA_C.CType.Array) _global.copyArray(dst.offset(i).read(), v, e);else dst.offset(i).write(v);
        }
        return dst;
    };
    _global.copyStruct2 = function copyStruct2(dst, src, vtype) {
        ...
        vtype.members.forEach(function (m, i) {
            ...
            if (m.vtype instanceof BA_C.CType.Struct) _global.copyStruct2(dst[k], val, m.vtype);else if (m.vtype instanceof BA_C.CType.Array) _global.copyArray(dst[k], val, m.vtype);else dst[k] = val;
        });
        return dst;
    };
    _global.pointer = function pointer(obj, key, vtype, ofs) {
        ...
        return {
            obj: obj,
            key: key,
            isValidBorder: function isValidBorder() {
    ...
    _global.promisize = function promisize(p) {
        if ((typeof AsyncByGenerator === "undefined" ? "undefined" : _typeof(AsyncByGenerator)) == "object") {
            if (root.AsyncByGenerator.supportsGenerator && root.AsyncByGenerator.isGenerator(p)) {
    ...
    _global.param_init = function param_init(arg, init) {
        var res = null;
        if (arg !== undefined && arg !== null) res = arg;else if (init !== undefined && init !== null) res = init;else throw "関数の引数が足りません。";
        return res;
    };
    ...
    _global.expandArray = function expandArray(aryptr, vtype, hasDust) {
        var a = aryptr.obj;
        while (a.length < vtype.length) {
            a.push(_global.initialValue(vtype.e, hasDust));
        }
    };
    ...
    var casts = {
        ...
        toUnsigned_int: function toUnsigned_int(param) {
            var res = 0;
            param += 0; //bool to int
            param &= 0xffffffff;
            res = param;
        ...
        toUnsigned_char: function toUnsigned_char(param) {
            var res = 0;
            param += 0; //bool to int
            param &= 0xffffffff;
        ...
        toUnsigned_float: function toUnsigned_float(param) {
            return param;
        },
        toDouble: function toDouble(param) {
            return param;
        },
        toUnsigned_double: function toUnsigned_double(param) {
            return param;
        },
        toArray: function toArray(param) {
            return param;
        },
        toLong: function toLong(param) {
            var res = 0;
            param += 0; //bool to int
            param &= 0xffffffff;
            res = param;
        ...
        toUnsigned_long: function toUnsigned_long(param) {
            var res = 0;
            param += 0; //bool to int
            param &= 0xffffffff;
            res = param;
        ...
        toShort: function toShort(param) {
            var res = 0;
            param += 0; //bool to int
            param &= 0xffff;

            if (param & 0x8000) res = param | 0xffff0000;else res = param;
            return res;
        },
        toUnsigned_short: function toUnsigned_short(param) {
            var res = 0;
            param += 0; //bool to int
            param &= 0xffff;
