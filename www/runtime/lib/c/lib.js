...
(function () {
	...
	_global.fgets = function fgets(str, len, fp) {
		...
		if (inputLen < 0) inputLen = heading.length;else inputLen++;
	...
	_global.scanf = function scanf() {
		...
		} else if (root.AsyncByGenerator && root.AsyncByGenerator.supportsGenerator && $("#console")[0]) {
			return new Promise(function (p) {
	...
	_global.sscanf = function sscanf() {
		...
		if (dests.length !== r.length) {
			throw new Error("scanfやfscanfで読んだ行に含まれる値の個数(" + r.length + ")と、" + "格納先の変数の個数(" + dests.length + ")が一致しません。");
		}
	...
	function sprintfJS() {
		...
		for (; i < s.length - 5; ++i) {
			...
			switch (s[i]) {
				...
				case "s":
					width = 0;v = _global.ch_ptr_to_str(getArg());if (precision) {
	...
	// also in ctrans.js
	["abs", "acos", "asin", "atan", "atan2", "ceil", "cos", "exp", "floor", "log", "max", "min", "pow", "random", "round", "sin", "sqrt", "tan"].forEach(function (k) {
		var f = Math[k];
