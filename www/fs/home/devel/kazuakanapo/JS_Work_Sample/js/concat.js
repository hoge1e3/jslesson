Tonyu.klass.define({
  ...
  methods: {
    main :function _trc_Colatz_main() {
      ...
      $LASTPOS=1000073;//user.Colatz:73
      _this.num=27;
      $LASTPOS=1000081;//user.Colatz:81
      while (_this.num!=1) {
        $LASTPOS=1000100;//user.Colatz:100
        if (_this.num%2==0) {
          $LASTPOS=1000122;//user.Colatz:122
          _this.num=_this.num/2;
          
        } else {
          $LASTPOS=1000152;//user.Colatz:152
          _this.num=_this.num*3+1;
          
        }
        $LASTPOS=1000175;//user.Colatz:175
        _this.addText("r",_this.num);
        $LASTPOS=1000197;//user.Colatz:197
        _this.addText("r","<br>");
  ...
  methods: {
    main :function _trc_PrintImages_main() {
      ...
      $LASTPOS=2000089;//user.PrintImages:89
      $LASTPOS=2000093;//user.PrintImages:93
      _this.i=1;
      while(_this.i<=50) {
        {
          $LASTPOS=2000113;//user.PrintImages:113
          if (_this.i%2==0) {
            $LASTPOS=2000133;//user.PrintImages:133
            _this.addText("area","<img src=images/neko1.png>");
            
          } else {
            $LASTPOS=2000198;//user.PrintImages:198
  ...
  methods: {
    main :function _trc_WhileDonyu_main() {
      ...
      $LASTPOS=3000073;//user.WhileDonyu:73
      $LASTPOS=3000077;//user.WhileDonyu:77
      _this.i=1;
      while(_this.i<=50) {
        {
          $LASTPOS=3000097;//user.WhileDonyu:97
          if (_this.i%2==0) {
            $LASTPOS=3000117;//user.WhileDonyu:117
            _this.addText("area","<img src=images/neko1.png>");
            
          } else {
            $LASTPOS=3000182;//user.WhileDonyu:182
            _this.addText("area","<img src=images/fish.png>");
